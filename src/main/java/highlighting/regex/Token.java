 ...
 * input text in the {@link Token#test(String, ColorResolver)} method. Each token has an associated colour used to
...
public final class Token {
  ...
  public List<HighlightRegion> test(String s, ColorResolver colorResolver) {
    return pattern.matcher(s).results().map(mr -> toRegion(mr, colorResolver)).collect(Collectors.toList());
  }
  ...
  private HighlightRegion toRegion(MatchResult mr, ColorResolver colorResolver) {
    return new HighlightRegion(mr.start(matchingGroup), mr.end(matchingGroup), colorResolver.resolveColor(colorType));
  }
